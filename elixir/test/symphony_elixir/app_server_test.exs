defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server translates stale workspace path failures into lifecycle errors when binding is closing" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-lifecycle-error-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(
        ...
        Jason.encode!(%{
          ...
          "issue_identifier" => "MT-LIFECYCLE",
          "run_instance_id" => "run-lifecycle-old",
          "worker_host" => nil,
      ...
      while IFS= read -r line; do
        ...
        case "$count" in
          ...
          3)
            ...
            ;;
          4)
            printf '%s\\n' '{"method":"turn/failed","params":{"message":"cwd missing","code":"ENOENT"}}'
            exit 0
            ;;
          *)
            exit 0
      ...
      issue = %Issue{
        ...
        description: "Translate stale workspace failures",
        state: "In Progress",
        url: "https://example.org/issues/MT-LIFECYCLE",
        labels: ["backend"]
      }

      assert {:error, {:workspace_lifecycle_invalid, details}} =
               AppServer.run(workspace, "Hit stale workspace", issue)

      assert details.reason == :resource_closing
      assert details.run_instance_id == "run-lifecycle-old"
      assert details.closing_reason == "terminal_cleanup_pending"
    after
      File.rm_rf(test_root)
    end
  end
