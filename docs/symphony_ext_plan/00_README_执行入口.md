...
- `DOCS/plan_rerun_fix/` 是 `M1-6 / C-21` 的专题参考集合，用于冻结 `turn/completed` 与错误续跑语义修正的边界、证据链与实现前提。
...
```text
你正在基于 OpenAI Symphony Elixir 原型机做二次开发。禁止换语言，禁止全量重写，禁止把多个项目塞进一个核心 Orchestrator。主线是：新增多项目 Control Plane，管理多个单项目 Symphony worker；每个 worker 仍读取自己的 WORKFLOW.generated.md、logs_root、workspace_root、内部 API port。Web 总览页只能读取轻量 summary，不允许默认拉取 raw event、完整 timeline、完整 prompt、完整 shell output。Codex 线程观测必须通过后端 state reducer 生成 phase/action/health，再给前端展示。每轮修改必须先按当前阶段要求执行：阶段 1 先完成创建仓库、阅读文档、确认方向和骨架搭建；从阶段 2 起必须先读当前源码和相关测试，并按《07_质量门禁与验收标准.md》先基于 `git diff` 判断验证级别，再给出设计差异、修改文件、与本轮验证级别匹配的验证命令或免测理由、回滚方式。开发阶段只跑定向测试；准备开 PR 或准备更新 PR 之前必须先跑 closeout gate，至少包含 `mix format --check-formatted`、`mix lint` 和改动面对应的定向测试，所有测试命令都必须显式带 `SYMPHONY_TEST_MAX_CASES=4`；若是 docs-only 或无可执行覆盖的改动，仍需跑 closeout gate，但可免定向测试并明确说明理由。`make -C elixir all` 只用于最终确认，不是日常开发命令，也不是复现工具；若 CI 失败，必须先看 CI 报错，再做本地定向检测和修复，修复完成后才能跑一次 `make -C elixir all` 做最终确认。重型测试全程必须监控资源并在结束后回收现场。不要生成小型任务列表，按本文档的阶段和质量门禁推进。
```
